...
export const MovementDashboardComp: React.FC = () => {
  // Define o ano atual dinamicamente com base na data do sistema
  const anoAtual = useMemo(() => dayjs().year(), []);

  // Monta a configuração dos meses dinamicamente para o ano corrente
  const MESES_CONFIG = useMemo(() => {
    return Array.from({ length: 12 }, (_, i) => {
      const mesObjeto = dayjs().year(anoAtual).month(i).date(1);
      return {
        chave: mesObjeto.format("MM"), // '01', '02', ..., '12'
        nome: mesObjeto.locale("pt-br").format("MMM"), // 'jan', 'fev', ...
        dataRef: mesObjeto.format("01/MM/YYYY"), // '01/01/2026', ...
      };
    });
  }, [anoAtual]);
  ...
  // PROCESSAMENTO DA MATRIZ PIVOTADA COM REPETIÇÃO DE PARCELAS AUTOMÁTICA
  const matrixData = useMemo(() => {
    ...
    // Inicializa todas as colunas do ano atual com valor zero
    ...
    dataRaw.forEach((item: any) => {
      ...
      // Se houver datafimmov, ela dita o limite. Se não, morre no mesmo mês
      ...
      for (let i = 0; i < mesesDuracao; i++) {
        ...
        // REGRA AUTOMÁTICA: computar apenas se a parcela estiver dentro do ano corrente
        if (dataReferenciaParcela.year() === anoAtual) {
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} sm={8}>
          <Card bordered={false} style={{ boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
            <Statistic
              title={`Previsão de Receita (${anoAtual})`}
        ...
        <Col xs={24} sm={8}>
          <Card bordered={false} style={{ boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
            <Statistic
              title={`Previsão de Despesas (${anoAtual})`}
      ...
      <Card 
        title={
          <Space>
            ...
            <span>Matriz de Fluxo de Caixa Consolidado (Modo Competência - Ano {anoAtual})</span>
          </Space>
        }
        bordered={false}
